...
import { Spin, Collapse, Empty, Button, Pagination, Input, message } from 'antd';
...
export const ProjectsList = ({ className, onAddClick, onMergeClick }) => {
  ...
  let content = (
    <Empty description="У вас нет загруженных проектов" />
  );

  if (isLoading) {
    content = (
      <div className={styles.spinner}><Spin delay={300} size="large" /></div>
    );
  }
